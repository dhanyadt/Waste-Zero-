...
// ✅ Pagination with numbered page buttons + ellipsis
...
const AdminOpportunities = () => {
  ...
  useEffect(() => { setPage(1); }, [search, statusFilter, ngoFilter, locationFilter]);

  const filtered = allOpps.filter(o => {
    const q = search.toLowerCase().trim();
    if (statusFilter && (o.status || "").toLowerCase() !== statusFilter) return false;
    if (ngoFilter) {
      const ngoName = (o.ngo?.name || o.createdBy?.name || "").toLowerCase();
      if (!ngoName.includes(ngoFilter.toLowerCase().trim())) return false;
    }
    if (locationFilter && !(o.location || "").toLowerCase().includes(locationFilter.toLowerCase().trim())) return false;
    if (q) {
      const searchable = [o.title, o.description, o.location, o.ngo?.name, o.createdBy?.name, o.status]
        ...
        <div style={{
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:14, alignItems:"flex-end" }}>
            ...
            <div style={{ minWidth:180 }}>
              <div style={{ fontSize:12, color: darkMode ? "#aaa" : T.textSoft, marginBottom:6 }}>NGO Name</div>
              <input value={ngoFilter} onChange={e => setNgoFilter(e.target.value)}
            ...
            <div style={{ minWidth:160 }}>
              <div style={{ fontSize:12, color: darkMode ? "#aaa" : T.textSoft, marginBottom:6 }}>Location</div>
