...
import { LayoutDashboard, LogOut, Camera, User, Target, Calendar, MessageSquare, TrendingUp, Settings, HelpCircle, Shield } from "lucide-react";
...
const S = {
  ...
  ring2: {
    ...
    bottom: 60, right: -70, pointerEvents: "none",
  },
  topGlow: {
  ...
  logoText: { fontFamily: serif, fontSize: 16, fontWeight: 800, color: "#fff", letterSpacing: "-.2px" },
  /* profile section */
  profileSection: {
  ...
  darkLabel: { fontSize: 13, color: "rgba(255,255,255,.5)", display: "flex", alignItems: "center", gap: 8 },
  logoutBtn: {
    display: "flex", alignItems: "center", gap: 10,
    width: "100%", padding: "9px 12px", borderRadius: 10,
    ...
    <div style={S.sidebar}>
      ...
      <div style={S.navSection}>
        ...
        {mainNav.map(({ label, path, icon }) => (
          <button key={path} style={S.navBtn(isActive(path))} onClick={() => navigate(path)}
            onMouseEnter={e => { if (!isActive(path)) e.currentTarget.style.background = "rgba(255,255,255,.06)"; e.currentTarget.style.color = "rgba(255,255,255,.9)"; }}
            onMouseLeave={e => { if (!isActive(path)) { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "rgba(255,255,255,.6)"; } }}>
      ...
      <div style={S.bottomSection}>
        <button style={S.logoutBtn}
          ...
          onMouseEnter={e => { e.currentTarget.style.background = "rgba(239,68,68,.1)"; e.currentTarget.style.color = "rgb(239,68,68)"; }}
